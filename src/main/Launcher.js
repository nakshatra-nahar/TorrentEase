...
export default class Launcher extends EventEmitter {
  ...
  init () {
    ...
    logger.info('[TorrentEase] openedAtLogin:', this.openedAtLogin)
   ...
   * "name": "TorrentEase Protocol",
   * "schemes": ["mo", "torrentease"]
   */
  handleOpenUrl () {
    if (is.mas() || !is.macOS()) {
      return
    ...
    app.on('open-url', (event, url) => {
      logger.info(`[TorrentEase] open-url: ${url}`)
  ...
  handleOpenFile () {
    ...
    app.on('open-file', (event, path) => {
      logger.info(`[TorrentEase] open-file: ${path}`)
      event.preventDefault()
      this.file = path
  ...
  handleAppLaunchArgv (argv) {
    logger.info('[TorrentEase] handleAppLaunchArgv:', argv)

    // args: array, extra: map
    const { args, extra } = splitArgv(argv)
    logger.info('[TorrentEase] split argv args:', args)
    logger.info('[TorrentEase] split argv extra:', extra)
  ...
  handelAppReady () {
    ...
    app.on('activate', () => {
      if (global.application) {
        logger.info('[TorrentEase] activate')
        global.application.showPage('index')
      }
    })
  }
  ...
  handleAppWillQuit () {
    app.on('will-quit', () => {
      logger.info('[TorrentEase] will-quit')
      if (global.application) {
        logger.info('[TorrentEase] will-quit.application.stop')
