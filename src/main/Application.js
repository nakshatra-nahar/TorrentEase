...
export default class Application extends EventEmitter {
  ...
  offConfigListeners () {
    try {
      ...
      logger.warn('[TorrentEase] offConfigListeners===>', e)
    }
    this.configListeners = {}
  }
  ...
  setupLogger () {
    ...
    this.configListeners[key] = userConfig.onDidChange(key, async (newValue, oldValue) => {
      logger.info(`[TorrentEase] detected ${key} value change event:`, newValue, oldValue)
  ...
  async stopEngine () {
    logger.info('[TorrentEase] stopEngine===>')
    try {
      await this.engineClient.shutdown({ force: true })
      logger.info('[TorrentEase] stopEngine.setImmediate===>')
      setImmediate(() => {
        this.engine.stop()
      })
    } catch (err) {
      logger.warn('[TorrentEase] shutdown engine fail: ', err.message)
    } finally {
      // no finally
    }
  }
  ...
  watchProtocolsChange () {
    ...
    this.configListeners[key] = userConfig.onDidChange(key, async (newValue, oldValue) => {
      ...
      logger.info('[TorrentEase] setup protocols client:', newValue)
  ...
  async startUPnPMapping () {
    ...
    try {
      ...
      logger.warn('[TorrentEase] start UPnP mapping fail', e.message)
    }
  }
  ...
  async stopUPnPMapping () {
    ...
    try {
      ...
      logger.warn('[TorrentEase] stop UPnP mapping fail', e)
    }
  }
  ...
  watchUPnPPortsChange () {
    ...
    watchKeys.forEach((key) => {
      this.configListeners[key] = systemConfig.onDidChange(key, async (newValue, oldValue) => {
        logger.info('[TorrentEase] detected port change event:', key, newValue, oldValue)
        ...
        try {
          ...
          logger.info('[TorrentEase] change UPnP port mapping failed:', e)
        }
      })
    })
  }
  ...
  watchUPnPEnabledChange () {
    ...
    this.configListeners[key] = userConfig.onDidChange(key, async (newValue, oldValue) => {
      logger.info('[TorrentEase] detected enable-upnp value change event:', newValue, oldValue)
      if (newValue) {
        this.startUPnPMapping()
      } else {
  ...
  syncTrackers (source, proxy) {
    ...
    setTimeout(() => {
      fetchBtTrackerFromSource(source, proxy).then((data) => {
        logger.warn('[TorrentEase] auto sync tracker data:', data)
        ...
        logger.warn('[TorrentEase] auto sync tracker failed:', err.message)
      })
    }, 500)
  }
  ...
  autoSyncTrackers () {
    ...
    logger.info('[TorrentEase] auto sync tracker checkIsNeedRun:', result)
    if (!result) {
      return
    }
  ...
  stop () {
    try {
      ...
      logger.warn('[TorrentEase] stop error: ', err.message)
    }
  }
  ...
  handleFile (filePath) {
    ...
    readFile(filePath, (err, data) => {
      if (err) {
        logger.warn(`[TorrentEase] read file error: ${filePath}`, err.message)
        return
      }
  ...
  async resetSession () {
    ...
    setTimeout(() => {
      unlink(sessionPath, (err) => {
        logger.info('[TorrentEase] Removed the download seesion file:', err)
      })
  ...
  savePreference (config = {}) {
    logger.info('[TorrentEase] save preference:', config)
    const { system, user } = config
    if (!isEmpty(system)) {
      console.info('[TorrentEase] main save system config: ', system)
      this.configManager.setSystemConfig(system)
      this.engineClient.changeGlobalOption(system)
    }

    if (!isEmpty(user)) {
      console.info('[TorrentEase] main save user config: ', user)
  ...
  handleCommands () {
    ...
    this.on('application:setup-protocols-client', (protocols) => {
      ...
      logger.info('[TorrentEase] setup protocols client:', protocols)
    ...
    this.on('application:reveal-in-folder', (data) => {
      const { gid, path } = data
      logger.info('[TorrentEase] application:reveal-in-folder===>', path)
      if (path) {
    ...
    this.on('help:official-website', () => {
      const url = 'https://torrentease.app/'
      this.openExternal(url)
    })

    this.on('help:manual', () => {
      const url = 'https://torrentease.app/manual'
      this.openExternal(url)
    })

    this.on('help:release-notes', () => {
      const url = 'https://torrentease.app/release'
      this.openExternal(url)
    })

    this.on('help:report-problem', () => {
      const url = 'https://torrentease.app/report'
      this.openExternal(url)
    })
  }
  ...
  handleIpcMessages () {
    ipcMain.on('command', (event, command, ...args) => {
      logger.log('[TorrentEase] ipc receive command', command, ...args)
      this.emit(command, ...args)
    })

    ipcMain.on('event', (event, eventName, ...args) => {
      logger.log('[TorrentEase] ipc receive event', eventName, ...args)
