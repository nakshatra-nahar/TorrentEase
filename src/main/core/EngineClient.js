...
export default class EngineClient {
  ...
  connect () {
    logger.info('[TorrentEase] main engine client connect', this.options)
  ...
  async call (method, ...args) {
    return this.client.call(method, ...args).catch((err) => {
      logger.warn('[TorrentEase] call client fail:', err.message)
    })
  }

  async changeGlobalOption (options) {
    logger.info('[TorrentEase] change engine global option:', options)
