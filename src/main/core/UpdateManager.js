...
export default class UpdateManager extends EventEmitter {
  constructor (options = {}) {
    ...
    logger.info('[TorrentEase] setup proxy:', this.options.proxy)
  ...
  setupProxy (proxy) {
    ...
    logger.info(`[TorrentEase] setup proxy: ${proxyRules}`, username, password, protocol, host, port)
    this.updater.netSession.setProxy({
      proxyRules
    })
  ...
  updateError (event, error) {
    ...
    this.updater.logger.warn(`[TorrentEase] update-error: ${msg}`)
