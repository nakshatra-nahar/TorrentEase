...
function initTrayWorker () {
  ...
  worker.addEventListener('message', (event) => {
    ...
    case 'initialized':
    case 'log':
      console.log('[TorrentEase] Log from Tray Worker: ', payload)
      break
    case 'tray:drawed':
      updateTray(payload)
      break
    default:
      console.warn('[TorrentEase] Tray Worker unhandled message type:', type, payload)
    }
  })
...
store.dispatch('preference/fetchPreference')
  .then((config) => {
    console.info('[TorrentEase] load preference:', config)
    init(config)
  })
  .catch((err) => {
    alert(err)
