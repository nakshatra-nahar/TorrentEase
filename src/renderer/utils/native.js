...
export const showItemInFolder = (fullPath, { errorMsg }) => {
  ...
  access(fullPath, constants.F_OK, (err) => {
    console.warn(`[TorrentEase] ${fullPath} ${err ? 'does not exist' : 'exists'}`)
    if (err && errorMsg) {
...
export const moveTaskFilesToTrash = (task) => {
  ...
  access(path, constants.F_OK, async (err) => {
    console.log(`[TorrentEase] ${path} ${err ? 'does not exist' : 'exists'}`)
    if (!err) {
  ...
  access(extraFilePath, constants.F_OK, async (err) => {
    console.log(`[TorrentEase] ${extraFilePath} ${err ? 'does not exist' : 'exists'}`)
    if (!err) {
